...
def draw(data1, data2):
    ...
    plt.plot(num, time1, marker=marker[0], color='#00838d', label='SuffixTree', lw=2, markersize=12)
    plt.plot(num, time2, marker=marker[1], color='#f28442', label='HashTable', lw=2, markersize=12)
    # ax = plt.gca()
    ...
    plt.savefig('calculateTreeKernel.png', dpi=300)


if __name__ == '__main__':
    data1 = clear_data(read_data('suffixTree.txt'))
    data2 = clear_data(read_data('hashTable.txt'))
    draw(data1, data2)
