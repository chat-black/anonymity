											 ...
											 IMDId *mdid)
{
	...
	// 首先确定这个 oid 代表什么类型的对象

	if (gpdb::IndexExists(oid))  // 根据 oid 判断索引是否存在，依赖 postgresql 底层提供的函数，下属函数与之相似
	{
...
//		给定一个 metadata 的 id，从 relcache 获得该 id 指定的索引
//
//---------------------------------------------------------------------------
IMDIndex *
										...
										IMDId *mdid_index)
{
	...
	if (NULL == index_rel)  // 没有该 id 指定的索引
	{
		GPOS_RAISE(gpdxl::ExmaMD, gpdxl::ExmiMDCacheEntryNotFound,
