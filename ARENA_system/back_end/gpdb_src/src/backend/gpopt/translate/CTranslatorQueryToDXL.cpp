...
CTranslatorQueryToDXL::TranslateSelectQueryToDXL()
{
	...
	// This entries is annotated(有注释的) as requiring SELECT permissions for the current user.
	...
	// construct CTEAnchor (anchor 锚) operators for the CTEs defined at the top level
	...
	if (NULL != m_query->setOperations)  // 如果集合操作不为空
	{
		List *target_list = m_query->targetList;
		// translate set operations  转换集合操作
		...
		ForEach(lc, target_list)
		{
			...
			resno++;
					  ...
					  m_query->windowClause))  // translate window clauses  集合操作为空，但是 windows 子句不为0
	{
	...
	{
		child_dxlnode = TranslateGroupingSets(  // 转换一个带有 group set 的查询
	...
	////////////////////////////////////////// 生成 output columns
	if (NULL == m_query->targetList)  // targetList 为空，创建空的 dxl_query_output_cols
	{
...
{
	CDXLNode *join_tree_dxlnode = TranslateFromExprToDXL(from_expr);  // 将 from 子句转换为了 DXL 

	// translate target list entries into a logical project
	// 将 target list 条目转变为逻辑投影
...
//		将一个带有 grouping分句的查询转变为一个 CDXLLogicalGroupBy 节点
//
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::CreateSimpleGroupBy(
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
	if (NULL == grpby_cols_bitset)  // has_aggs 为 false，不存在聚合语句
	{
		GPOS_ASSERT(!has_aggs);
		if (!has_grouping_sets)  // 如果不存在 groupint_sets
		{
			// no group by needed and not part of a grouping sets query:
			// propagate child columns to output columns
			// 没有 group by 语句，并且不是一个 grouping sets查询的一部分
			// 将一个子列传播到一个输出列
		...
		// 在有 grouping sets 的查询中，我们会创建一个分支负责 GB 的 grouping sets
		// 在没有将子列转变为 输出 hash map时，后面的处理会介绍 NULL
...
CTranslatorQueryToDXL::TranslateGroupingSets(
	...
	IntToUlongMap *output_attno_to_colid_mapping)
{
	const ULONG num_of_cols = gpdb::ListLength(target_list) + 1;  // columns 的数量

	if (NULL == group_clause)  // 一般情况下
	{
		IntToUlongMap *child_attno_colid_mapping =
			GPOS_NEW(m_mp) IntToUlongMap(m_mp);  // int -> long
		...
		CBitSet *bitset = NULL;
		if (has_aggs)  // 存在聚合函数，会为 bitset 分配内存
		{
			bitset = GPOS_NEW(m_mp) CBitSet(m_mp);
		}

		// in case of aggregates(聚合函数的情形), construct a group by operator
	...
	// grouping 函数指向 grouping 列的位置，因此在处理 grouping分句时构造一个映射 pos->grouping column
...
//---------------------------------------------------------------------------
//	@function:
//		CTranslatorQueryToDXL::TranslateFromExprToDXL  // 函数名可以理解为将 From子句 转变为 DXL 
//													   // 其中 From子句 中可以存在 join 也可以不存在
//	@doc:
//		Translate the FromExpr on a GPDB query into either a CDXLLogicalJoin
//		or a CDXLLogicalGet
// 
//		将 GPDB query 中的 from表达式转变成 CDXLLogicalJoin 或者是 CDXLLogicalGet
//
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateFromExprToDXL(FromExpr *from_expr)
{
	...
	if (0 == gpdb::ListLength(from_expr->fromlist))  // fromlist 是空的
	{
		dxlnode = DXLDummyConstTableGet();
	}
	else
	{
		if (1 == gpdb::ListLength(from_expr->fromlist))  // from子句中只有一个关系
		{
			Node *node = (Node *) gpdb::ListNth(from_expr->fromlist, 0);  // 取得第1个元素
			GPOS_ASSERT(NULL != node);
		...
		else
		{
			...
			// 在 DXL 中，我们用一个条件为 true 的内连接代表一个 n-ary 的 join
			// 其中 join conditions 代表 FromExpr->quals 已经被转换为一个 CDXLLogicalSelect 在 CDXLLogicalJoin 的
			// 顶层
	...
	// translate the quals
	Node *qual_node = from_expr->quals;  // 修饰语
	CDXLNode *condition_dxlnode = NULL;  // 条件DXL
	if (NULL != qual_node)  // 存在条件
	{
		condition_dxlnode = TranslateExprToDXL((Expr *) qual_node);
	}

	if (1 >= gpdb::ListLength(from_expr->fromlist))  // fromlist 的长度小于等于1，条件不会涉及两个表的链接
	{
		if (NULL != condition_dxlnode)  // 存在条件
		{
	...
	else  //n-ary joins  n路 join
	{
		if (NULL == condition_dxlnode)  // 没有条件，全连接
		{
...
//		返回一个 CDXLNode 代表一个 from 子句条目，它既可以是一个 fromlist 条目也可以是 JoinExpr 的左孩子或者右孩子
//
//---------------------------------------------------------------------------
CDXLNode *
CTranslatorQueryToDXL::TranslateFromClauseToDXL(Node *node)
{
	GPOS_ASSERT(NULL != node);  // 这个节点不为空

	if (IsA(node, RangeTblRef))  // 范围表
	{
		RangeTblRef *range_tbl_ref = (RangeTblRef *) node;  // 强制类型转换
...
//---------------------------------------------------------------------------
CDXLNodeArray *
CTranslatorQueryToDXL::CreateDXLOutputCols(   // 所有信息都是可知的
	List *target_list, IntToUlongMap *attno_to_colid_mapping) const
{
	GPOS_ASSERT(NULL != target_list);
	GPOS_ASSERT(NULL != attno_to_colid_mapping);

	CDXLNodeArray *dxlnodes = GPOS_NEW(m_mp) CDXLNodeArray(m_mp);  // 构造最终返回的数据结构

	ListCell *lc = NULL;
	ForEach(lc, target_list)  // 对其中的每一行进行处理
	{
		TargetEntry *target_entry = (TargetEntry *) lfirst(lc);
		GPOS_ASSERT(0 < target_entry->resno);
		ULONG resno = target_entry->resno;  // 这个关系在查询中的位置，从1开始
...
////////////////////////////////////////////////// OK
		GPOS_ASSERT(NULL != target_entry);
		CMDName *mdname = NULL;  // 构造 colname
		if (NULL == target_entry->resname)
		{
		...
		// 构造 colid，需要首先构造 attno_to_colid_mapping
		const ULONG colid =
			CTranslatorUtils::GetColId(resno, attno_to_colid_mapping);

		// create a column reference  构造列的参考
		IMDId *mdid_type = GPOS_NEW(m_mp)
			CMDIdGPDB(gpdb::ExprType((Node *) target_entry->expr));  // 返回 OID
		INT type_modifier = gpdb::ExprTypeMod((Node *) target_entry->expr);  // 返回 int32

		// reference 中包含所有的必要信息 (需要的信息为 mdname, colid, mdid_type, type_modifier)
		...
		dxlnodes->Append(dxlnode);  // 添加一个节点
	}
