...
//		它存储着一个 group expression 在给定 optimization context 下的 cost
//		也就是说，每个 cost context 是与某个 optimization context 相对应的，而一个 group expression
//		可以在不同 optimization context 下使用，此时在每个 optimization context 下都会有对应的 cost context
//---------------------------------------------------------------------------
...
private:
	...
	// group expression to be used stats derivation(派生) during costing
	// 在 costing 阶段，用于状态派生的 group expression
	CGroupExpression *m_pgexprForStats;

	// array of optimization contexts of child groups
	// 所有子 group 的优化上下文构成的队列（每个子group有一个特定的 optimization context）
	...
	// flag to indicate if cost context is pruned (被修剪的),
...
public:
	...
	// 该 cost context 所处的 optimization context
	...
	CCost CostCompute(CMemoryPool *mp, std::vector<CCost *> & pdrgpcostChildren);  // 用于 ARENA 系统，减少了内存的分配和释放
