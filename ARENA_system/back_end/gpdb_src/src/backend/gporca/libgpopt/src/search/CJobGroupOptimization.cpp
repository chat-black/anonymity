...
void
CJobGroupOptimization::Init(
	CGroup *pgroup, CGroupExpression *pgexprOrigin,	
		// group expression that triggered optimization job,  触发这个 optimization 工作的 expression
	...
	COptimizationContext *poc)
{
	...
	m_poc = m_pgroup->PocInsert(poc);  // 插入 optimizationContext
	if (poc == m_poc)
	{
		// pin down context in hash table
		m_poc->AddRef();
	}
	SetJobQueue(m_poc->PjqOptimization());  // m_poc-> 返回 optimization job queue accessor（访问器）
											 ...
											 CJob *pjOwner)
{
	...
	if (!pgroup->FImplemented())  // 如果还没有进行 Implemented
	{
...
{
	CJob *pj = psc->Pjf()->PjCreate(CJob::EjtGroupOptimization);  // SchedulerContext -> JobFactory -> 构造 job

	// initialize job
	CJobGroupOptimization *pjgo = PjConvert(pj);  // 类型转换
	pjgo->Init(pgroup, pgexprOrigin, poc);  // 设置 job 的状态机以及 job 队列等内容
	psc->Psched()->Add(pjgo, pjParent);  // psc->Psched() (scheduler) -> Add (添加新的需要执行的 job)
}
