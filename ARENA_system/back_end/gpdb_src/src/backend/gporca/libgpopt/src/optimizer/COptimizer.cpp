...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
// static void Optimizer::WritePlan(CExpression * pexprPlan)
// {
//     std::ofstream fout("/tmp/plantree.txt");
//     if (fout.open())
//     {
//         fout << std::endl << "Physical plan: " << std::endl << *pexprPlan;
//         fout.close();
//     }
...
)
{
	...
	// 如果需要进行 minidump，打开 minidump 文件并且初始化 minidumper（即使不进行 dump，也会创造 minidumper对象，
	// 但是不会进行初始化，它始终保持非活跃状态）
