...
//---------------------------------------------------------------------------
void *
CScheduler::Run(void *pv)  // 静态方法: 无法访问普通变量
{
	CSchedulerContext *psc = reinterpret_cast<CSchedulerContext *>(pv);
	psc->Psched()->ExecuteJobs(psc);  // psc(调度器上下文) -> Psched() 获取调度器 -> ExecuteJobs(psc) 调度器执行
...
//		只要word queue中仍然存在可以执行的 job ，就持续运行
//
//---------------------------------------------------------------------------
void
...
CScheduler::Schedule(CJob *pj)
{
	...
	pjl->Init(pj);  // 利用 job 初始化相应的 job link
	...
	// add to waiting list
	m_listjlWaiting.Push(pjl);  // job link 加入到等待队列中
