...
CJobGroupExploration::FScheduleGroupExpressions(CSchedulerContext *psc)
{
	...
	// 从第一个 expression 进行遍历，如果需要就对他们进行调度
	CGroupExpression *pgexpr = PgexprFirstUnsched();  // 取得第一个未调度的 expression
	while (NULL != pgexpr)  // expression 不为空
	{
	...
	BOOL fNewJobs = (m_pgexprLastScheduled != pgexprLast);  // 是否找到了新的 job
