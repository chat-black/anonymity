			 ...
			 CCostContext::HashValue, CCostContext::Equals> *ptmap)
{
	...
	CCostContextArray *pdrgpcc = pgexprCurrent->PdrgpccLookupAll(mp, poc);  // 寻找所有满足要求的 cost context (cost context 与 expression 和 optimization context相对应)
	...
	for (ULONG ulCC = 0; ulCC < ulCCSize; ulCC++)
	{
		...
		CCostContext *pccCurrent = (*pdrgpcc)[ulCC];  // 取得对应的 CostContext ，这样就意味着每个 expression 实际上对应多个形式
		if (NULL != pccParent)
		{
		...
		COptimizationContextArray *pdrgpoc = pccCurrent->Pdrgpoc();  // 队列中的每个 OptimizationContext 对应一个子 Group
		if (NULL != pdrgpoc)
		{
			// process children recursively
			const ULONG arity = pgexprCurrent->Arity();  // 子 group 的队列
			for (ULONG ul = 0; ul < arity; ul++)
			{
				GPOS_CHECK_ABORT;
				...
				if (!pgroupChild->FScalar())  // 不是标量操作符（Scalar)
				{
					pocChild = (*pdrgpoc)[ul];
					GPOS_ASSERT(NULL != pocChild);
				}
				pgroupChild->BuildTreeMap(mp, pocChild, pccCurrent, ul, ptmap);  // 每个子 Group 调用 BuildTreeMap
			}
		}
	}
...
)
{
	...
	while (NULL != pgexprCurrent)
	{
		...
		else
		{
			...
			// this is a scalar group, link parent cost context to group's dummy(虚假的) context
			ptmap->Insert(pccParent, child_index, PccDummy(), true);
