...
//		返回给定 hash table 元素的 MD provider
//
//---------------------------------------------------------------------------
IMDProvider *
CMDAccessor::SMDProviderElem::Pmdp()
...
//		对于给定的资源 id ，注册一个 MD provider
//
//---------------------------------------------------------------------------
void
CMDAccessor::RegisterProvider(CSystemId sysid, IMDProvider *pmdp)
{
	CAutoP<SMDProviderElem> a_pmdpelem;  //  智能指针，自动释放内存
	a_pmdpelem = GPOS_NEW(m_mp) SMDProviderElem(sysid, pmdp);

	MDPHTAccessor mdhtacc(m_shtProviders, *(a_pmdpelem.Value()));  // (MDPHT , SMDProviderElem) -> MDPHT: CSyncHashtable<SMDProviderElem, SMDPorviderElem>
	// MDPHTAccessor == CSyncHashtableAccessByKey<SMDAccessorElem, MdidPtr>
	//		SMDAccessorElem 是一个结构体
	//		CSyncHashtableAccessByKey

	// insert provider in the hash table
	mdhtacc.Insert(a_pmdpelem.Value());  // .Value() 函数取得真实的指针
	a_pmdpelem.Reset();
}
...
CMDAccessor::Pmdp(CSystemId sysid)
{
...

	{
		...
		MDPHTAccessor mdhtacc(m_shtProviders, mdpelem);  // m_shtProviders: MD providers 的哈希表   (MDPHT, SMDProviderElem)
...
//
//		从元数据缓存中获得一个元数据缓存对象，有可能需要先从外部元数据提供者处获得再将其
//		存储。
//---------------------------------------------------------------------------
...
CMDAccessor::GetImdObj(IMDId *mdid)
{
	...
	CTimerUser timerLookup;	 // timer to measure lookup time  测量查询时间的寄存器
	if (fPrintOptStats)
	{
		timerLookup.Restart();
	}
	...
	if (NULL == pimdobj)
	{
		...
		// 局部哈希表中没有定位到该对象，尝试在 MD 缓存中查找
		//  a_pmdcacc 与 m_shtCacheAccessors 的区别是什么呢？**************************************************

		// construct a key for cache lookup
		IMDProvider *pmdp = Pmdp(mdid->Sysid());  // Sysid 获得系统 id    /// 
		...
		if (NULL == pmdobjNew)
		{
			...
			// 这个对象不在 MD 缓存中，从 MD provider 中获得该对象
			...
			// 对于 CTAS mdid，我们不会将其加入到相应对象的  MD cache 中，因为这些对象都有固定的id
			// 并且如果允许缓存，并且这些对象被缓存，之后一系列的 CTAS 查询将会试图使用缓存的对象，
			// 它具有不同的模式，这将会导致问题。因此，对于这些对象，我们跳过 MD cache 的步骤，将其从
			// MD provider 处获得，直接存储到局部哈希表中
			...
			if (IMDId::EmdidGPDBCtas != mdid->MdidType())
			{
				...
				// 无论插入成功还是失败，对象都得到固定，因为这个对象已经在缓存中了
...
//
//		给定 type 的 info 和 source system id，从 md cache 中获取 MD type
//---------------------------------------------------------------------------
const IMDType *
...
{
	ULONG nentries = m_shtCacheAccessors.Size();  // 条目的数量
	...
	// while iterating.
	// 迭代哈希表并且将所有的条目插入到队列中，迭代器会在哈希表上掌控一个锁，因此
	// 我们一定不能做任何重大的事情（例如：分配内存）
	cacheEntries = GPOS_NEW_ARRAY(m_mp, IMDCacheObject *, nentries);
	aCacheEntries = cacheEntries;
	{
		MDHTIter mdhtit(m_shtCacheAccessors);  // m_shtCacheAccessors 缓存访问器的哈希表
		ul = 0;
