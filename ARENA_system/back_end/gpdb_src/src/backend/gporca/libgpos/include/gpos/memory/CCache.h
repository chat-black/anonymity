...
//		The current implementation has a fixed gclock based eviction policy(回收策略).
//
//		缓存的定义：
//		Cache 存储着 cached objects 的 键-值对，Keys是经过哈希过的值，哈希函数通过函数指针
//		pfuncHash 指定. Key 值的相等性判断通过函数 pfuncEqual。 cached object 的值包含对象的值
//		作为其成员。
//
//		Cache 的 API 允许客户存储，查询，删除以及迭代缓存的对象
//
//		Cache 只能通过 CCacheAccessor 友元类进行访问。当前的实现基于回收策略具有固定的 gclock
//
//---------------------------------------------------------------------------
template <class T, class K>
class CCache
