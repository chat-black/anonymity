 ...
 * -----------------------------------
 */
char ARENA_file_name [70];
void arena_get_hash(StringInfo input_message)
{
	int length = strlen(input_message->data);
	if (length > 70 && strncmp(&(input_message->data[length - 71]), "ARENA:", 6) == 0)  // 只有大于71才有可能是包含 hash 信息的请求，格式为： 查询 ARENA:sha256;
	{
		strlcpy(ARENA_file_name, &(input_message->data[length - 65]), 65);  // 将目标文件名复制进全局变量中
		input_message->data[length - 72] = ';';
		input_message->data[length - 71] = '\0';
		input_message->len -= 71;
	}
}
 ...
 * ----------------------------------------------------------------
 */
static int	InteractiveBackend(StringInfo inBuf);  // StringInfo 是指向StringInfoData的指针
