 ...
 *
 * resname is required to represent the correct column name in non-resjunk (junk 垃圾，破烂)
 ...
 * ordinary cross-product(全连接) join ("FROM foo, bar, baz WHERE ...").
 ...
 * aliases and so on.  The output column set is implicitly(隐含的) just the union(并集)
...
typedef struct FromExpr
{
	...
	Node	   *quals;			/* qualifiers on join, if any (join 的修饰词)*/
} FromExpr;
